...
struct SeqNote
{
  std::string over1, over2, text;
}; // over2 nonempty → range
struct SeqGroup
{
  std::string type, label;
  int start_event = 0, end_event = 0;
};
...
struct StateDiagram
{
  ...
  std::vector<StateTrans> transitions;
};
...
{
  std::string type, name;
  bool pk = false, fk = false;
};
struct EREntity
{
  std::string name;
  std::vector<ERAttr> attrs;
};
struct ERRel
{
  std::string e1, e2, label, card1, card2;
};
struct ERDiagram
{
  std::vector<EREntity> entities;
  std::vector<ERRel> relations;
};
...
struct GanttTask
{
  std::string id, name, after;
  int start_day = 0, dur = 1;
...
{
  std::string title;
  std::vector<GanttSection> sections;
};
...
struct Requirement
{
  std::string type, name, id, text, risk, method;
};
struct ReqElement
{
  std::string name, type, docref;
};
struct ReqRel
{
  std::string from, to, reltype;
};
...
struct GitCommit
{
  std::string id, tag, branch, merge_from;
  enum class T
  {
    Normal,
    Reverse,
...
struct ArchService
{
  std::string id, icon, label, group;
};
struct ArchGroup
{
  std::string id, icon, label;
};
struct ArchEdge
{
  std::string from, to;
...
struct ArchDiagram
{
  ...
  std::vector<ArchService> services;
...
struct EventModelingDiagram
{
  std::string title;
  std::vector<EMItem> items;
...
// ── Ishikawa (Fishbone) ──────────────────────────────────────────────────────
struct IshikawaCause
{
  std::string text;
  std::vector<IshikawaCause> sub;
};
struct IshikawaCategory
{
  std::string name;
  std::vector<IshikawaCause> causes;
};
struct IshikawaDiagram
{
  std::string effect;
  std::vector<IshikawaCategory> categories;
};
...
{
  std::string name;
  float visibility = 0.5f, evolution = 0.5f;
};
struct WardleyLink
{
  std::string from, to;
};
struct WardleyDiagram
{
  std::string title;
  std::vector<WardleyComp> components;
