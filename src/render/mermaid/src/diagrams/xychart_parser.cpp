...
{
  if(key == "showLabel")
    cfg.show_label = parse_bool_value(value, cfg.show_label);
  else if(key == "labelPadding")
    cfg.label_padding = parse_float_value(value, cfg.label_padding);
  else if(key == "showTitle")
    cfg.show_title = parse_bool_value(value, cfg.show_title);
  else if(key == "titlePadding")
    cfg.title_padding = parse_float_value(value, cfg.title_padding);
  else if(key == "showTick")
    cfg.show_tick = parse_bool_value(value, cfg.show_tick);
  else if(key == "tickLength")
    cfg.tick_length = parse_float_value(value, cfg.tick_length);
  else if(key == "tickWidth")
    cfg.tick_width = parse_float_value(value, cfg.tick_width);
  else if(key == "showAxisLine")
    cfg.show_axis_line = parse_bool_value(value, cfg.show_axis_line);
  else if(key == "axisLineWidth")
    cfg.axis_line_width = parse_float_value(value, cfg.axis_line_width);
}
...
static void parse_xychart_frontmatter(std::string_view src, XYDiagram &out)
{
  ...
  while(p < src.size())
  {
    ...
    if(key == "width")
      out.config.width = std::max(120.0f, parse_float_value(value, out.config.width));
    else if(key == "height")
      out.config.height = std::max(120.0f, parse_float_value(value, out.config.height));
    else if(key == "showTitle")
      out.config.show_title = parse_bool_value(value, out.config.show_title);
    else if(key == "showDataLabel")
      out.config.show_data_label = parse_bool_value(value, out.config.show_data_label);
    else if(key == "showDataLabelOutsideBar")
      out.config.show_data_label_outside_bar = parse_bool_value(value, out.config.show_data_label_outside_bar);
    else if(key == "titlePadding")
      out.config.title_padding = parse_float_value(value, out.config.title_padding);
    else if(key == "plotReservedSpacePercent")
      out.config.plot_reserved_space_percent = parse_float_value(value, out.config.plot_reserved_space_percent);
