...
bool parse_sequence(std::string_view src, SequenceDiagram &out)
{
  ...
  while(L.next(line))
  {
    ...
    if(!header)
    {
      if(starts_with(ll, "sequencediagram"))
      {
        header = true;
        continue;
      }
    ...
    if(starts_with(ll, "title "))
    {
      out.title = std::string(trim(tline.substr(6)));
      continue;
    }
    ...
    }
    {
      ...
      if(seq_split_arrow(tline, lhs, rhs, lbl))
      {
        ...
        {
          to.pop_back();
          out.events.push_back({SequenceDiagram::Event::T::Activate, -1, "", "", to});
        }
        if(!to.empty() && to.back() == '-')
        {
          to.pop_back();
          out.events.push_back({SequenceDiagram::Event::T::Deactivate, -1, "", "", to});
        }
        ensure_part(from);
        ensure_part(to);
