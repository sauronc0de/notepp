...
bool parse_requirement(std::string_view src, RequirementDiagram &out)
{
  ...
  while(L.next(line))
  {
    ...
    {
      if(sw(ll, "id: "))
        cur_req.id = std::string(tr(line.substr(4)));
      else if(sw(ll, "text: "))
        cur_req.text = std::string(tr(line.substr(6)));
      else if(sw(ll, "risk: "))
        cur_req.risk = std::string(tr(line.substr(6)));
      else if(sw(ll, "verifymethod: ") || sw(ll, "verify: "))
        cur_req.method = std::string(tr(line.substr(ll.find(':') + 2)));
      continue;
    }
    if(in_elem)
    {
      if(sw(ll, "type: "))
        cur_elem.type = std::string(tr(line.substr(6)));
      else if(sw(ll, "docref: "))
        cur_elem.docref = std::string(tr(line.substr(8)));
      continue;
    }
