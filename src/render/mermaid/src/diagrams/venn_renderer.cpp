...
void render_venn(const VennDiagram &d, int id)
{
  ...
  for(int i = 0; i < ns; ++i)
  {
    ...
    {
      if(i == 0)
        lp = ImVec2(set_centers[0].x - ts.x * 0.5f, set_centers[0].y - r - ts.y - 2);
      else if(i == 1)
        lp = ImVec2(set_centers[1].x - ts.x - r * 0.15f, set_centers[1].y + r * 0.55f);
      else
        lp = ImVec2(set_centers[2].x + r * 0.15f, set_centers[2].y + r * 0.55f);
    }
    else
    {
