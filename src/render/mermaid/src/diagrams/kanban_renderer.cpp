...
void render_kanban(const KanbanDiagram &d, int id)
{
  ...
  if(es.drag_active)
  {
    ...
    for(int i = 0; i < nc; ++i)
    {
      ...
      if(mouse.x >= cx && mouse.x < cx + col_w)
      {
        ...
        for(int j = 0; j < nj; ++j)
        {
          if(mouse.y < body_top + j * (card_h + vgap) + card_h * 0.5f)
          {
            es.drop_ri = j;
            break;
          }
  ...
  if(kb_hovered && !es.drag_active && ImGui::IsMouseClicked(ImGuiMouseButton_Right))
  {
    ...
    if(hov_ci >= 0 && hov_ri >= 0)
    {
      ...
      std::strncpy(es.edit_label, card.label.c_str(), 255);
      es.edit_label[255] = '\0';
      std::strncpy(es.edit_desc, card.description.c_str(), 511);
      es.edit_desc[511] = '\0';
      es.edit_focus = true;
  ...
  if(es.drag_active && es.drag_ci >= 0 && es.drag_ci < static_cast<int>(d.columns.size()) && es.drag_ri >= 0 && es.drag_ri < static_cast<int>(d.columns[es.drag_ci].cards.size()))
  {
  ...
  {
    if(es.ctx_ci >= 0 && es.ctx_ci < static_cast<int>(d.columns.size()) && es.ctx_ri >= 0 && es.ctx_ri < static_cast<int>(d.columns[es.ctx_ci].cards.size()))
    {
