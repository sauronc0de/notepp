...
ExprResult ExprParser::parse_comparison()
{
  ...
  else
  {
    ...
    {
    case CmpOp::Gt:
      result.boolean = lhs.value.number > rhs.value.number;
      break;
    case CmpOp::Lt:
      result.boolean = lhs.value.number < rhs.value.number;
      break;
...
enum class CrossInventoryOpKind
{
  None,
  Remove,
  ReplaceWith
...
void render_inventory_hover_popup(const std::string &popup_id, const InventorySlotInfo &slot, float popup_content_width)
{
  ...
  if(ImGui::BeginTooltip())
  {
    ...
    if(!slot.title.empty())
    {
      const bool quantity_fits_inline = slot.quantity && ImGui::CalcTextSize(slot.title.c_str()).x + style.ItemSpacing.x + ImGui::CalcTextSize(slot.quantity->c_str()).x <= popup_content_width;
...
void draw_inventory_slot_preview(const InventorySlotInfo &slot, const ImVec2 &min, const ImVec2 &max, bool selected, bool hovered, float cell_size = 48.0f)
{
  ...
  ImVec4 border_color = selected
                        ...
                        : slot.has_mark_color ? slot.mark_color
                                              : ImGui::GetStyleColorVec4(hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Border);
    ...
    for(int row = 0; row < rows; ++row)
    {
      for(int col = 0; col < cols; ++col)
      {
        ...
        if(ImGui::BeginPopupContextItem("##slot_menu"))
        {
          ...
          if(state.has_mark_color)
          {
            const float hex_color_width = ImGui::GetFrameHeight() + ImGui::GetStyle().ItemInnerSpacing.x + ImGui::CalcTextSize("#00EDFF").x + ImGui::GetStyle().FramePadding.x * 2.0f;
                       ...
                       std::unordered_map<std::string, std::string> &replacements, std::vector<std::string> &errors)
{
    ...
    if(sv->kind == ValueKind::Array)
    {
      for(const Value &stroke_val : sv->array)
      {
        ...
        if(const Value *f = find_object_field(stroke_val, "color"))
          ...
          {
            std::string dummy;
            parse_inventory_mark_color(f->str.c_str(), dummy, si.color);
          }
  ...
  {
    zoom = pan.def_zoom;
    offset_x = pan.def_offset_x;
    offset_y = pan.def_offset_y;
  }
  ...
  if(ImGui::BeginChild(child_id.c_str(), ImVec2(widget_width, requested_height), true, child_flags))
  {
    ...
    if(hovered_marker >= 0 && canvas_hovered && editing_marker_idx == -1 && !mdrag.active && !ds.draw_mode && !ds.erase_mode)
    {
    ...
    if(!readonly && canvas_hovered && ImGui::GetIO().MouseWheel != 0.f)
    {
      ...
      pan.deferred = true;
      pan.def_zoom = zoom;
      pan.def_offset_x = offset_x;
      pan.def_offset_y = offset_y;
      pan.zoom_settle = 4; // write after 4 idle frames
    }

    // Marker drag — start on left-click over a marker (not in draw mode)
    if(!readonly && !ds.draw_mode && canvas_hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Left) && hovered_marker >= 0)
    {
      mdrag.active = true;
    ...
    if(mdrag.active)
    {
      ...
      else
      {
        ...
        if(markers_arr && markers_arr->kind == ValueKind::Array && mdrag.marker_idx < static_cast<int>(markers_arr->array.size()))
        {
          Value &mv = markers_arr->array[static_cast<size_t>(mdrag.marker_idx)];
          {
            Value f;
            f.kind = ValueKind::Number;
            f.number = mdrag.live_uv.x;
            upsert_object_field(mv, "x", std::move(f));
          }
          {
            Value f;
            f.kind = ValueKind::Number;
            f.number = mdrag.live_uv.y;
            upsert_object_field(mv, "y", std::move(f));
          }
          changed = true;
        }
        mdrag.active = false;
    ...
    if(ds.is_drawing && ds.draw_mode)
    {
      ...
      else
      {
        ...
        if(ds.current_pts.size() >= 2)
        {
          ...
          Value stroke_obj;
          stroke_obj.kind = ValueKind::Object;
          {
            Value f;
            f.kind = ValueKind::String;
            f.str = serialize_stroke_pts(ds.current_pts);
            upsert_object_field(stroke_obj, "pts", std::move(f));
          }
          {
            Value f;
            f.kind = ValueKind::String;
            f.str = hex_buf;
            upsert_object_field(stroke_obj, "color", std::move(f));
          }
          {
            Value f;
            f.kind = ValueKind::Number;
            f.number = uv_t;
            upsert_object_field(stroke_obj, "t", std::move(f));
          }
    ...
    if(!readonly && !ds.draw_mode && !ds.erase_mode && !mdrag.active && canvas_hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Left) && hovered_marker < 0)
    {
      pan.active = true;
    ...
    if(ImGui::BeginPopup(marker_popup_id.c_str()))
    {
      ...
      ImGui::InputText("Title", ed.title, sizeof(ed.title));
      ImGui::SetNextItemWidth(220.f);
      ImGui::InputText("Image", ed.image, sizeof(ed.image));
      ImGui::SetNextItemWidth(220.f);
      ...
      ImGui::InputFloat("X", &ed.x, 0.f, 0.f, "%.4f");
      ImGui::SameLine();
      ImGui::SetNextItemWidth(90.f);
      ImGui::InputFloat("Y", &ed.y, 0.f, 0.f, "%.4f");
      ...
      preset_color("Green", 0x58 / 255.f, 0xC4 / 255.f, 0x72 / 255.f);
      ImGui::SameLine();
      preset_color("Blue", 0x57 / 255.f, 0xA7 / 255.f, 1.00f);
      ImGui::SameLine();
      preset_color("Amber", 1.00f, 0xB3 / 255.f, 0x47 / 255.f);
      ImGui::SameLine();
      preset_color("Red", 1.00f, 0x6B / 255.f, 0x6B / 255.f);
      ImGui::SameLine();
      preset_color("Purple", 0xC5 / 255.f, 0x7B / 255.f, 1.00f);

      ImGui::Separator();
      const bool apply_enter = ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && ImGui::IsKeyPressed(ImGuiKey_Enter, false) && !desc_active;
    ...
    if(ImGui::BeginPopup(map_ctx_popup_id.c_str()))
    {
      ...
      {
        Value img_val;
        img_val.kind = ValueKind::String;
        img_val.str = trim(std::string(ss.image));
      ...
      {
        zoom = std::clamp(zoom * 1.25f, 1.0f, 50.f);
        changed = true;
      }
      ImGui::SameLine();
      if(ImGui::Button("Zoom -"))
      {
        zoom = std::clamp(zoom * 0.8f, 1.0f, 50.f);
        changed = true;
      }
      ImGui::SameLine();
  ...
  {
    {
      ...
      v.number = zoom;
      upsert_object_field(updated, "zoom", std::move(v));
    }
    {
      Value v;
      v.kind = ValueKind::Number;
      v.number = offset_x;
      upsert_object_field(updated, "offset_x", std::move(v));
    }
    {
      Value v;
      v.kind = ValueKind::Number;
      v.number = offset_y;
      upsert_object_field(updated, "offset_y", std::move(v));
    }
    set_override(ctx, block, *var_name, updated, replacements, errors);
...
void render_statement(EvalContext &ctx, const ParsedBlock &block, const Statement &stmt, std::unordered_map<std::string, std::string> &replacements, std::vector<std::string> &errors)
{
  ...
  case Statement::Kind::Assignment: {
    ...
    if(!stmt.index_expr.empty())
    {
      ...
      if(!arr.error.empty())
      {
        errors.push_back(arr.error);
        break;
      }
      if(arr.value.kind != ValueKind::Array)
      {
        errors.push_back("'" + stmt.name + "' is not an array");
        break;
      }
      ExprResult idx = ctx.evaluate(stmt.index_expr);
      if(!idx.error.empty())
      {
        errors.push_back(idx.error);
        break;
      }
      if(idx.value.kind != ValueKind::Number || !idx.value.is_integer)
      {
        errors.push_back("array index must be an integer");
        break;
      }
      const int i = static_cast<int>(idx.value.number);
      ...
      if(actual < 0 || actual >= sz)
      {
        errors.push_back("array index out of bounds");
        break;
      }
