...
void render_kanban(const KanbanDiagram &d, int id)
{
  ...
  int nc=(int)d.columns.size();
  int max_cards=0; for(auto &c:d.columns) max_cards=std::max(max_cards,(int)c.cards.size());
  float cw=nc*(col_w+hgap)+hgap+pad*2;
  float ch=col_header_h+(max_cards+1)*(card_h+vgap)+pad*2;
  const ImVec2 orig=ImGui::GetCursorScreenPos();
  ...
  for(int i=0;i<nc;++i){
    ...
    ImVec2 ls=ImGui::CalcTextSize(d.columns[i].label.c_str());
    dl->AddText(ImVec2(x+(col_w-ls.x)*0.5f,y+(col_header_h-ls.y)*0.5f),tcol,d.columns[i].label.c_str());
    // column body
    dl->AddRectFilled(ImVec2(x,y+col_header_h),ImVec2(x+col_w,y+ch-pad*2),series_color(i,0.08f),0);
    dl->AddRect(ImVec2(x,y),ImVec2(x+col_w,y+ch-pad*2),bord,4);
    float cy=y+col_header_h+vgap;
    for(auto &card:d.columns[i].cards){
      dl->AddRectFilled(ImVec2(x+4,cy),ImVec2(x+col_w-4,cy+card_h),fill,3);
      dl->AddRect(ImVec2(x+4,cy),ImVec2(x+col_w-4,cy+card_h),bord,3);
