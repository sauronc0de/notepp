...
public:
  ScopedXdgConfig()
  {
    ...
    const std::string base = (fs::temp_directory_path() /
        ("notepp_project_tests_" + std::to_string(static_cast<long long>(++counter)))).generic_string();
  ...
  ~ScopedXdgConfig()
  {
    if(prev_) setenv("XDG_CONFIG_HOME", previous_value_.c_str(), 1);
...
void test_recent_projects_dedupe_and_cap()
{
  ...
  for(int i = 0; i < 12; ++i)
  {
    const fs::path p = (i % 3 == 0) ? a : (i % 3 == 1) ? b : c;
